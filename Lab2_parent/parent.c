...
int main(int argc, char* argv[], char* envp[]) {
    ...
    setlocale(LC_COLLATE, "C"); //set local of region settings

    int i = 0;
    while (envp[i++]);
    qsort(envp, i-1, sizeof(char*), compare); //вывод переменных в сортировонном виде
    i = 0;
    while(envp[i]) {
        printf("%s\n", envp[i++]);
    }
    ...
    while(1) {
        ...
        if (key == 'q')
            ...
            if (chpid == 0) {
                ...
                switch (key) {
                    case '+':
                        childDir = getenv("CHILD_PATH"); //pc from func
                        break;
                    case '*':
                        childDir = scanEnvp(envp); //arg command line
                        break;
                    case '&':
                        childDir = scanEnvp(environ); //from extern
                        break;
                }
                if (childDir) {
